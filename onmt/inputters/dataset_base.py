...
class Dataset(TorchtextDataset):
    ...
    def __init__(self, fields, readers, data, dirs, sort_key,
        ...
        for ex_dict in starmap(_join_dicts, zip(*read_iters)):
            ...
            # add by wchen for src_utr_position field
            if "src_utr_position" in fields:
                ex_dict["src_utr_position"] = ex_dict["src"]
            # add by wchen for src_word_utr_ids field
            if "src_word_utr_ids" in fields:
                ex_dict["src_word_utr_ids"] = ex_dict["src"]
            # add by wchen for tgt_support_utrs field
            if "tgt_support_utrs" in fields and 'tgt' in ex_dict:
                ex_dict["tgt_support_utrs"] = {'src': ex_dict["src"], 'tgt': ex_dict["tgt"]}
            # add by wchen for tgt facts
            if "tgt_fact_triplets" in fields and 'tgt' in ex_dict:
                ex_dict["tgt_fact_triplets"] = ex_dict["tgt"]

            ex_fields = {k: [(k, v)] for k, v in fields.items() if
                         k in ex_dict}
            ex = Example.fromdict(ex_dict, ex_fields)

            # check the utr_posi consistency bewtween the  "src_utr_position" and "tgt_support_utrs"
            if hasattr(ex, "src_utr_position") and hasattr(ex, "tgt_support_utrs"):
                utr_posis = ex.src_utr_position
                support_utrs_info = ex.tgt_support_utrs
                support_utrs_info = json.loads(support_utrs_info)
                for tgt_i_info in support_utrs_info:
                    for utr_idx, utr_posi in zip(tgt_i_info['utr_idxs'], tgt_i_info['utr_posis']):
                        assert utr_posis[utr_idx] == utr_posi, "The position of the {}th utterance is inconsistent " \
                                                               "between the position {} from src_utr_position and " \
                                                               "the position {} from tgt_support_utrs".format(utr_idx,
                                                                                                              utr_posis[utr_idx],
                                                                                                              utr_posi)
