...
def process_one_shard(corpus_params, params):
    corpus_type, fields, src_reader, src_coref_reader, tgt_reader, opt, existing_fields,\
        src_vocab, tgt_vocab = corpus_params
    i, (src_shard, src_coref_shard, tgt_shard, maybe_id, filter_pred) = params
    # create one counter per shard
    sub_sub_counter = defaultdict(Counter)
    assert len(src_shard) == len(tgt_shard)
    assert len(src_shard) == len(src_coref_shard)
    logger.info("Building shard %d." % i)
    dataset = inputters.Dataset(
        fields,
        readers=([src_reader, src_coref_reader, tgt_reader]
                 if tgt_reader else [src_reader, src_coref_reader]),
        data=([("src", src_shard), ("src_coref", src_coref_shard), ("tgt", tgt_shard)]
              if tgt_reader else [("src", src_shard), ("src_coref", src_shard)]),
...
def build_save_dataset(corpus_type, fields, src_reader, src_coref_reader, tgt_reader, opt):
    assert corpus_type in ['train', 'valid']

    if corpus_type == 'train':
        counters = defaultdict(Counter)
        srcs = opt.train_src
        # add by wchen for src_corefs
        src_corefs = opt.train_src_coref
        tgts = opt.train_tgt
        ids = opt.train_ids
        ...
        srcs = [opt.valid_src]
        # add by wchen for src_corefs
        src_corefs = [opt.valid_src_coref]
        tgts = [opt.valid_tgt]
        ids = [None]
    ...
    def shard_iterator(srcs, src_corefs, tgts, ids, existing_shards,
                       existing_fields, corpus_type, opt):
        """
        Builds a single iterator yielding every shard of every corpus.
        """
        for src, src_coref, tgt, maybe_id in zip(srcs, src_corefs, tgts, ids):
            ...
            if ((corpus_type == "train" or opt.filter_valid)
                ...
                filter_pred = partial(
                    ...
                    min_src_len=opt.min_src_len)
            else:
                filter_pred = None
            src_shards = split_corpus(src, opt.shard_size)
            # add by wchen for src_corefs
            src_coref_shards = split_corpus(src_coref, opt.shard_size)
            tgt_shards = split_corpus(tgt, opt.shard_size)
            for i, (ss, scs, ts) in enumerate(zip(src_shards, src_coref_shards, tgt_shards)):
                yield (i, (ss, scs, ts, maybe_id, filter_pred))

    shard_iter = shard_iterator(srcs, src_corefs, tgts, ids, existing_shards,
                                existing_fields, corpus_type, opt)

    with Pool(opt.num_threads) as p:
        dataset_params = (corpus_type, fields, src_reader, src_coref_reader, tgt_reader,
...
def preprocess(opt):
    ...
    fields = inputters.get_fields(
        ...
        utr_select_num=opt.utr_select_num,
        jaccard_sim_th=opt.jaccard_sim_th)

    src_reader = inputters.str2reader[opt.data_type].from_opt(opt)
    # add by wchen for src_coref
    src_corefs_reader = inputters.str2reader[opt.data_type].from_opt(opt)
    tgt_reader = inputters.str2reader["text"].from_opt(opt)

    logger.info("Building & saving training data...")
    build_save_dataset(
        'train', fields, src_reader, src_corefs_reader, tgt_reader, opt)

    if opt.valid_src and opt.valid_tgt:
        logger.info("Building & saving validation data...")
        build_save_dataset('valid', fields, src_reader, src_corefs_reader, tgt_reader, opt)
