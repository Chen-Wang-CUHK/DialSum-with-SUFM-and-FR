...
def make_learning_rate_decay_fn(opt):
    ...
    elif opt.decay_method == 'valid_ppl_up':
        return functools.partial(
            valid_ppl_up_decay, rate=opt.learning_rate_decay)
...
def valid_ppl_up_decay(do_decay, cur_lr, rate):
    if do_decay:
        return max([cur_lr * rate, 1e-5])
    else:
        return cur_lr
...
class Optimizer(object):
    ...
    def __init__(self,
       ...
       Args:
        ...
        self._decayed_learning_rate = learning_rate
        self._last_ppl = None
    ...
    def learning_rate(self, cur_ppl=None):
        """Returns the current learning rate."""
        if self._learning_rate_decay_fn is None:
            return self._learning_rate
        elif self._learning_rate_decay_fn.func is valid_ppl_up_decay:
            if cur_ppl is None:
                return self._decayed_learning_rate
            do_decay = cur_ppl > self._last_ppl if self._last_ppl is not None else False
            self._last_ppl = cur_ppl
            self._decayed_learning_rate = self._learning_rate_decay_fn(do_decay, self._decayed_learning_rate)
            return self._decayed_learning_rate
        else:
    ...
    def step(self, cur_ppl=None):
        """Update the model parameters based on current gradients.

        Optionally, will employ gradient modification or update learning
        rate.
        """
        learning_rate = self.learning_rate(cur_ppl)
