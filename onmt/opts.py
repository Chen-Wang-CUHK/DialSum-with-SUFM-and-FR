...
def model_opts(parser):
    ...
    group.add('--encoder_type', '-encoder_type', type=str, default='rnn',
              choices=['rnn', 'brnn', 'seq_hre_brnn', 'mean', 'transformer', 'cnn'],
              help="Type of encoder layer to use. Non-RNN layers "
                   "are experimental. Options are "
                   "[rnn|brnn|seq_hre_brnn|mean|transformer|cnn].")
    group.add('--decoder_type', '-decoder_type', type=str, default='rnn',
              choices=['rnn', 'seq_hre_ifrnn', 'transformer', 'cnn'],
              help="Type of decoder layer to use. Non-RNN layers "
                   "are experimental. Options are "
                   "[rnn|seq_hre_ifrnn|transformer|cnn].")
    ...
    # add by wchen, src_coreference layer
    group = parser.add_argument_group('Model- coreference layer')
    group.add('--coref_layer_type', '-coref_layer_type', type=str, default='none',
              choices=['none', 'attn', 'max_pool', 'attn_noself', 'attn_weighted', 'attn_noself_weighted', 'attn_no_extra_params'],
              help="The type of the coreference layer")
    group.add('--coref_fnn_all', '-coref_fnn_all', action="store_true",
              help='Besides the corefs, also aplly a fnn layer to other hidden states of the memory bank')
    # add by wchen, SeqHREModel options
    group = parser.add_argument_group('Model-SeqHREModel layer')
    group.add('--utr_mrg_type', '-utr_mrg_type', type=str, default='ave_pool', choices=['max_pool', 'ave_pool', 'fw_bw_cat'],
              help="The type of the merge operation from word-level representation to utterance-level representation")
    group.add('--enc_final_state_type', '-enc_final_state_type', type=str, default='word', choices=['word', 'utr'],
              help="Use the word-level or the utr-level final state to initialize the decoder")
    group.add('--hr_attn_type', '-hr_attn_type', type=str, default='utr_word_both',
              choices=['word_only', 'utr_word_both'],
              help="""Type of hierarchical attention. Options are [utr_only|word_only|utr_word_both]""")
    group.add('--seqHRE_attn_rescale', '-seqHRE_attn_rescale', action="store_true",
              help='Whether to recale the word level attention using the utterance level attention for the seqHRE type models.')
    group.add('--rm_middle_dp', '-rm_middle_dp', action="store_true",
              help='Whether to remove the dropout layer between the word-level encoding and the utr-level encoding.')
    # add by wchen, support_utr_loss and previous_utr_loss
    group.add('--lambda_support_utr', '-lambda_support_utr', type=float, default=0.0,
              help='Lambda value for lambda_support_utr loss')
    group.add('--lambda_previous_utr', '-lambda_previous_utr', type=float, default=0.0,
              help='Lambda value for lambda_previous_utr loss')
    group.add('--lambda_tgt_fact_re', '-lambda_tgt_fact_re', type=float, default=0.0,
              help='Lambda value for lambda_tgt_fact_re loss')
    group.add('--tgt_transE_state_type', '-tgt_transE_state_type', type=str, default='out_h_after_dp',
              choices=['out_h_after_dp', 'out_h_before_dp', 'state_h'],
              help="""Type of decoder state to compute the tgt fact transE loss.
              Options are [out_h_after_dp|out_h_before_dp|state_h]""")
...
def preprocess_opts(parser):
    ...
    # add by wchen
    # ---
    group.add('--train_src_coref', '-train_src_coref', required=True, nargs='+',
              help="Path to the training source coreference data")
    group.add('--valid_src_coref', '-valid_src_coref', default=None,
              help="Path to the validation source coreference data")
    group.add('--utr_select_num', '-utr_select_num', type=int, default=2,
              help="Maximum number of selected supporting utterances.")
    group.add('--jaccard_sim_th', '-jaccard_sim_th', type=float, default=0.15,
              help="The Jaccard score threshold for selecting the supporting utterances.")
    # ---
    ...
    group.add('--min_src_len', '-min_src_len', type=int, default=1,
...
def train_opts(parser):
    ...
    group.add('--decay_method', '-decay_method', type=str, default="none",
              choices=['noam', 'noamwd', 'rsqrt', 'none', 'valid_ppl_up'],
...
def translate_opts(parser):
    ...
    group.add('--src', '-src', required=True,
              help="Source sequence to decode (one line per "
                   "sequence)")
    # add by wchen for src corederence
    group.add('--src_coref', '-src_coref', required=True,
              help="Path to the testing source coreference data")
    # add by wchen for tgt step feat
    group.add('--step_feat_vocab', '-step_feat_vocab', action='store_true',
              help="Whether to use step feature for the tgt when decoding.")
    ...
    group.add('--log_file_level', '-log_file_level', type=str,
              ...
              choices=StoreLoggingLevelAction.CHOICES,
              default="0")
    group.add('--attn_score_folder', '-attn_score_folder', type=str, default=None,
              help='If not none, output the best attention scores to this folder')
